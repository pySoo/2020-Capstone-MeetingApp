...
class PlaceChoiceActivity constructor() : AppCompatActivity(), View.OnClickListener {
    ...
    private var layout_placechoice: View? = null
    protected override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_place_choice)
        layout_placechoice = findViewById<View>(R.id.layout_placechoice)
    ...
    public override fun onClick(v: View) {
        when (v.getId()) {
                                ...
                                if (task.isSuccessful()) {
                                    ...
                                    if (document.exists()) {
                                        ...
                                        if (document.getData().get("userID").size != 1) {
                                            ...
                                            val snackbar: Snackbar = Snackbar.make(layout_placechoice, "모임원이 1명일 때 중간지점을 찾을 수 없어요", Snackbar.LENGTH_INDEFINITE)
            ...
            (R.id.searchBtn) -> {
                myStartActivity(SearchPlaceActivity::class.java, scheduleInfo)
                finish()
            }
            (R.id.voteBtn) -> {
                myStartActivity(VoteActivity::class.java, scheduleInfo)
                finish()
            }
        }
    }
