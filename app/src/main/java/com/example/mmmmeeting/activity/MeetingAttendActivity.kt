...
class MeetingAttendActivity constructor() : AppCompatActivity() {
    var attend: Button? = null
    var db: FirebaseFirestore = FirebaseFirestore.getInstance()
    protected override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_meeting_attend)
        attend = findViewById<Button>(R.id.attendBtn)
        attend!!.setOnClickListener(object : View.OnClickListener {
    ...
    private fun checkCode() {
        val code: String = (findViewById<View>(R.id.attendCode) as EditText).getText().toString()
    ...
    private fun updateUser(code: String) {
        // 2. code 존재시 code document -> userID -> 현재 UID add
        val user: FirebaseUser = FirebaseAuth.getInstance().getCurrentUser()
        val newUser: DocumentReference = db.collection("meetings").document(code)
