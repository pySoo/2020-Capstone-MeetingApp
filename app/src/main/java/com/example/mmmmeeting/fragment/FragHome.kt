...
class FragHome constructor() : Fragment() {
    private var firebaseFirestore: FirebaseFirestore? = null
    private var scheduleAdapter: ScheduleAdapter? = null
    private var postList: ArrayList<ScheduleInfo>? = null
    ...
    public override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val recyclerView: RecyclerView = view.findViewById(R.id.recyclerView_sc)
        name = view.findViewById<View>(R.id.schedule_name) as TextView?
        name.setText("약속 목록")
            ...
            public override fun onRefresh() {
                recyclerView.postDelayed(object : Runnable {
                    public override fun run() {
                        postList = ArrayList<ScheduleInfo>()
                        scheduleAdapter = ScheduleAdapter(getActivity(), postList)
        ...
        view.findViewById<View>(R.id.write_schedule).setOnClickListener(onClickListener)
        recyclerView.setHasFixedSize(true)
        recyclerView.setLayoutManager(LinearLayoutManager(getActivity()))
        recyclerView.setAdapter(scheduleAdapter)
    ...
    var onClickListener: View.OnClickListener = object : View.OnClickListener {
        public override fun onClick(v: View) {
            when (v.getId()) {
                R.id.write_schedule -> myStartActivity(MakeScheduleActivity::class.java)
            }
        }
    }
    ...
    private fun postsUpdate(clear: Boolean) {
        ...
        val collectionReference: CollectionReference = firebaseFirestore.collection("schedule")
                        ...
                        if (task.isSuccessful()) {
                            ...
                            for (document: QueryDocumentSnapshot in task.getResult()) {
                                ...
                                if ((document.getData().get("meetingID").toString() == meetingCode)) {
                                    ...
                                    val temp: ScheduleInfo = ScheduleInfo(
                                    ...
                                    if (document.get("meetingPlace") != null) {
                                        temp.setMeetingPlace(document.get("meetingPlace").get("name").toString())
                                    }
