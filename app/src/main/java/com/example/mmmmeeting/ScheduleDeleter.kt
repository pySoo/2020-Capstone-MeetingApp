...
class ScheduleDeleter {
    ...
    private var onPostListener: OnScheduleListener? = null
    ...
    fun storageDelete(postInfo: ScheduleInfo) {
        ...
        for (i in contentsList.indices) {
            ...
            if (Util.isStorageUrl(contents)) {
                successCount++
                val desertRef: StorageReference = storageRef.child("schedule/" + id + "/" + Util.storageUrlToName(contents))
    ...
    private fun storeDelete(id: String, postInfo: ScheduleInfo) {
        ...
        if (successCount == 0) {
            firebaseFirestore.collection("schedule").document(id)
                    ...
                    .addOnSuccessListener(object : OnSuccessListener<Void?> {
                        public override fun onSuccess(aVoid: Void?) {
                            Util.showToast(activity, "약속을 삭제하였습니다.")
                    ...
                    .addOnFailureListener(object : OnFailureListener {
                        public override fun onFailure(e: Exception) {
                            Util.showToast(activity, "약속을 삭제하지 못하였습니다.")
                        }
                    })
        }
    }
